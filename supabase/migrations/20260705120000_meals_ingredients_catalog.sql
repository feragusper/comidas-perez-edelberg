...
--
-- 1. Tabla `ingredients`: catálogo de ingredientes reutilizables.
-- 2. Tabla `meals`: catálogo unificado de comidas (estáticas + custom),
--    cada comida referencia sus ingredientes por `ingredient_ids`.
-- 3. Copia de `custom_meals` -> `meals` (custom_meals queda intacta
--    hasta terminar el refactor del frontend).
-- 4. Seed de ingredientes base + catálogo estático componentizado.
--
-- El historial de `meal_plan` NO se toca: los snapshots embebidos se
-- resuelven por id contra este catálogo en tiempo de lectura.
-- ============================================================
...
CREATE TABLE public.meals (
  ...
  tags text[] NOT NULL DEFAULT '{}',
  -- Referencias a ingredients.ingredient_id. Vacío = comida sin normalizar.
  ingredient_ids text[] NOT NULL DEFAULT '{}',
  created_at timestamptz NOT NULL DEFAULT now()
);

-- ── Permisos + RLS (mismo patrón que custom_meals/pantry) ───
...
-- (custom_meals sigue existiendo hasta que el frontend migre;
--  estas comidas quedan sin ingredient_ids = pendientes de normalizar)
